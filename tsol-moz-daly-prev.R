### TSOL MOZAMBIQUE
### -- disability adjusted life years / prevalence
...
## NCC-epilepsy prevalent cases, age-sex specific [10 rows]
P_ncc_ep_act <- t(apply(t(c(pop_mx)), 2, function(x) x * prev_ncc_ep_act))
str(P_ncc_ep_act)

## NCC-epilepsy prevalent cases - treated, age-sex specific [10 rows]
P_ncc_ep_act_tr <- t(apply(P_ncc_ep_act, 1, function(x) x * prop_ep_trt))
t(apply(P_ncc_ep_act_tr, 1, mean_ci))

## NCC-epilepsy prevalent cases - non-treated, age-sex specific [10 rows]
P_ncc_ep_act_nt <- t(apply(P_ncc_ep_act, 1, function(x) x * (1-prop_ep_trt)))
t(apply(P_ncc_ep_act_nt, 1, mean_ci))
...
## NCC-epilepsy-migraine prevalent cases, age-sex specific [10 rows]
P_ncc_ep_mgr <- t(apply(t(c(pop_mx)), 2, function(x) x * prev_ncc_ep_mgr))
t(apply(P_ncc_ep_mgr, 1, mean_ci))

## NCC-epilepsy-ttha prevalent cases, age-sex specific [10 rows]
P_ncc_ep_tns <- t(apply(t(c(pop_mx)), 2, function(x) x * prev_ncc_ep_tns))
t(apply(P_ncc_ep_tns, 1, mean_ci))
...
yld_ep_tr[1, 1, ] <-
  burden(N = P_ncc_ep_act_tr[1, ], DW = dsw_ep_tr,
         A = ep_ons[1], L = ep_dur, K = K, r = r, a = ep_ons[1])
yld_ep_tr[2, 1, ] <-
  burden(N = P_ncc_ep_act_tr[2, ], DW = dsw_ep_tr,
         A = ep_ons[2], L = ep_dur, K = K, r = r, a = ep_ons[2])
yld_ep_tr[3, 1, ] <-
  burden(N = P_ncc_ep_act_tr[3, ], DW = dsw_ep_tr,
         A = ep_ons[3], L = ep_dur, K = K, r = r, a = ep_ons[3])
yld_ep_tr[4, 1, ] <-
  burden(N = P_ncc_ep_act_tr[4, ], DW = dsw_ep_tr,
         A = ep_ons[4], L = ep_dur, K = K, r = r, a = ep_ons[4])
yld_ep_tr[5, 1, ] <-
  burden(N = P_ncc_ep_act_tr[5, ], DW = dsw_ep_tr,
         A = ep_ons[5], L = ep_dur, K = K, r = r, a = ep_ons[5])

yld_ep_tr[1, 2, ] <-
  burden(N = P_ncc_ep_act_tr[6, ], DW = dsw_ep_tr,
         A = ep_ons[1], L = ep_dur, K = K, r = r, a = ep_ons[1])
yld_ep_tr[2, 2, ] <-
  burden(N = P_ncc_ep_act_tr[7, ], DW = dsw_ep_tr,
         A = ep_ons[2], L = ep_dur, K = K, r = r, a = ep_ons[2])
yld_ep_tr[3, 2, ] <-
  burden(N = P_ncc_ep_act_tr[8, ], DW = dsw_ep_tr,
         A = ep_ons[3], L = ep_dur, K = K, r = r, a = ep_ons[3])
yld_ep_tr[4, 2, ] <-
  burden(N = P_ncc_ep_act_tr[9, ], DW = dsw_ep_tr,
         A = ep_ons[4], L = ep_dur, K = K, r = r, a = ep_ons[4])
yld_ep_tr[5, 2, ] <-
  burden(N = P_ncc_ep_act_tr[10, ], DW = dsw_ep_tr,
         A = ep_ons[5], L = ep_dur, K = K, r = r, a = ep_ons[5])
...
yld_ep_nt[1, 1, ] <-
  burden(N = P_ncc_ep_act_nt[1, ], DW = dsw_ep_nt,
         A = ep_ons[1], L = ep_dur, K = K, r = r, a = ep_ons[1])
yld_ep_nt[2, 1, ] <-
  burden(N = P_ncc_ep_act_nt[2, ], DW = dsw_ep_nt,
         A = ep_ons[2], L = ep_dur, K = K, r = r, a = ep_ons[2])
yld_ep_nt[3, 1, ] <-
  burden(N = P_ncc_ep_act_nt[3, ], DW = dsw_ep_nt,
         A = ep_ons[3], L = ep_dur, K = K, r = r, a = ep_ons[3])
yld_ep_nt[4, 1, ] <-
  burden(N = P_ncc_ep_act_nt[4, ], DW = dsw_ep_nt,
         A = ep_ons[4], L = ep_dur, K = K, r = r, a = ep_ons[4])
yld_ep_nt[5, 1, ] <-
  burden(N = P_ncc_ep_act_nt[5, ], DW = dsw_ep_nt,
         A = ep_ons[5], L = ep_dur, K = K, r = r, a = ep_ons[5])

yld_ep_nt[1, 2, ] <-
  burden(N = P_ncc_ep_act_nt[6, ], DW = dsw_ep_nt,
         A = ep_ons[1], L = ep_dur, K = K, r = r, a = ep_ons[1])
yld_ep_nt[2, 2, ] <-
  burden(N = P_ncc_ep_act_nt[7, ], DW = dsw_ep_nt,
         A = ep_ons[2], L = ep_dur, K = K, r = r, a = ep_ons[2])
yld_ep_nt[3, 2, ] <-
  burden(N = P_ncc_ep_act_nt[8, ], DW = dsw_ep_nt,
         A = ep_ons[3], L = ep_dur, K = K, r = r, a = ep_ons[3])
yld_ep_nt[4, 2, ] <-
  burden(N = P_ncc_ep_act_nt[9, ], DW = dsw_ep_nt,
         A = ep_ons[4], L = ep_dur, K = K, r = r, a = ep_ons[4])
yld_ep_nt[5, 2, ] <-
  burden(N = P_ncc_ep_act_nt[10, ], DW = dsw_ep_nt,
         A = ep_ons[5], L = ep_dur, K = K, r = r, a = ep_ons[5])
...
yld_ha_mgr[1, 1, ] <-
  burden(N = P_ncc_ep_mgr[1, ], DW = dsw_mgr,
         A = ha_ons[1], L = ha_dur, K = K, r = r, a = ha_ons[1])
yld_ha_mgr[2, 1, ] <-
  burden(N = P_ncc_ep_mgr[2, ], DW = dsw_mgr,
         A = ha_ons[2], L = ha_dur, K = K, r = r, a = ha_ons[2])
yld_ha_mgr[3, 1, ] <-
  burden(N = P_ncc_ep_mgr[3, ], DW = dsw_mgr,
         A = ha_ons[3], L = ha_dur, K = K, r = r, a = ha_ons[3])
yld_ha_mgr[4, 1, ] <-
  burden(N = P_ncc_ep_mgr[4, ], DW = dsw_mgr,
         A = ha_ons[4], L = ha_dur, K = K, r = r, a = ha_ons[4])
yld_ha_mgr[5, 1, ] <-
  burden(N = P_ncc_ep_mgr[5, ], DW = dsw_mgr,
         A = ha_ons[5], L = ha_dur, K = K, r = r, a = ha_ons[5])

yld_ha_mgr[1, 2, ] <-
  burden(N = P_ncc_ep_mgr[6, ], DW = dsw_mgr,
         A = ha_ons[1], L = ha_dur, K = K, r = r, a = ha_ons[1])
yld_ha_mgr[2, 2, ] <-
  burden(N = P_ncc_ep_mgr[7, ], DW = dsw_mgr,
         A = ha_ons[2], L = ha_dur, K = K, r = r, a = ha_ons[2])
yld_ha_mgr[3, 2, ] <-
  burden(N = P_ncc_ep_mgr[8, ], DW = dsw_mgr,
         A = ha_ons[3], L = ha_dur, K = K, r = r, a = ha_ons[3])
yld_ha_mgr[4, 2, ] <-
  burden(N = P_ncc_ep_mgr[9, ], DW = dsw_mgr,
         A = ha_ons[4], L = ha_dur, K = K, r = r, a = ha_ons[4])
yld_ha_mgr[5, 2, ] <-
  burden(N = P_ncc_ep_mgr[10, ], DW = dsw_mgr,
         A = ha_ons[5], L = ha_dur, K = K, r = r, a = ha_ons[5])
...
yld_ha_tns[1, 1, ] <-
  burden(N = P_ncc_ep_tns[1, ], DW = dsw_tns,
         A = ha_ons[1], L = ha_dur, K = K, r = r, a = ha_ons[1])
yld_ha_tns[2, 1, ] <-
  burden(N = P_ncc_ep_tns[2, ], DW = dsw_tns,
         A = ha_ons[2], L = ha_dur, K = K, r = r, a = ha_ons[2])
yld_ha_tns[3, 1, ] <-
  burden(N = P_ncc_ep_tns[3, ], DW = dsw_tns,
         A = ha_ons[3], L = ha_dur, K = K, r = r, a = ha_ons[3])
yld_ha_tns[4, 1, ] <-
  burden(N = P_ncc_ep_tns[4, ], DW = dsw_tns,
         A = ha_ons[4], L = ha_dur, K = K, r = r, a = ha_ons[4])
yld_ha_tns[5, 1, ] <-
  burden(N = P_ncc_ep_tns[5, ], DW = dsw_tns,
         A = ha_ons[5], L = ha_dur, K = K, r = r, a = ha_ons[5])

yld_ha_tns[1, 2, ] <-
  burden(N = P_ncc_ep_tns[6, ], DW = dsw_tns,
         A = ha_ons[1], L = ha_dur, K = K, r = r, a = ha_ons[1])
yld_ha_tns[2, 2, ] <-
  burden(N = P_ncc_ep_tns[7, ], DW = dsw_tns,
         A = ha_ons[2], L = ha_dur, K = K, r = r, a = ha_ons[2])
yld_ha_tns[3, 2, ] <-
  burden(N = P_ncc_ep_tns[8, ], DW = dsw_tns,
         A = ha_ons[3], L = ha_dur, K = K, r = r, a = ha_ons[3])
yld_ha_tns[4, 2, ] <-
  burden(N = P_ncc_ep_tns[9, ], DW = dsw_tns,
         A = ha_ons[4], L = ha_dur, K = K, r = r, a = ha_ons[4])
yld_ha_tns[5, 2, ] <-
  burden(N = P_ncc_ep_tns[10, ], DW = dsw_tns,
         A = ha_ons[5], L = ha_dur, K = K, r = r, a = ha_ons[5])
...
## cases, deaths
mean_ci(colSums(P_ncc_ep_act))
mean_ci(colSums(P_ncc_ep_mgr))
mean_ci(colSums(P_ncc_ep_tns))
...
## YLDs, YLLs, DALYs per prevalent epilepsy case
mean_ci(yld_ep_all / colSums(P_ncc_ep_act))
mean_ci(yld_ha_all / colSums(P_ncc_ep_act))
mean_ci(yld_all / colSums(P_ncc_ep_act))
mean_ci(yll_ep_all / colSums(P_ncc_ep_act))
mean_ci(daly_all / colSums(P_ncc_ep_act))
mean_ci(daly_ep_all / colSums(P_ncc_ep_act))
